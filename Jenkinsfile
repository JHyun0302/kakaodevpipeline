pipeline {
  ...
  stages {
    stage('깃 업데이트') {
      steps {
        git url: 'https://github.com/JHyun0302/kakaodevpipeline.git', branch: 'main'
      }
    }
    ...
    stage('쿠버네티스 디플로리 서비스') {
      steps {
         sh '''
           ssh 211.183.3.100 'kubectl create deploy deploy-yellow --image=jhyun302/kakaodev:yellow'
